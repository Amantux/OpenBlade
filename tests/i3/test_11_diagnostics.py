...
class TestEvents:
    def test_events_endpoint_is_reachable(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/events", headers=auth_headers)
        assert resp.status_code == 200

    def test_events_response_is_list_or_object(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/events", headers=auth_headers)
        assert resp.status_code == 200
...
class TestFirmware:
    def test_firmware_endpoint_is_reachable(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/firmware", headers=auth_headers)
        assert resp.status_code in (200, 404)

    def test_firmware_version_reported(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/firmware", headers=auth_headers)
...
class TestRASTickets:
    def test_ras_endpoint_is_reachable(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
