...
class TestInventory:
    ...
    def test_inventory_slot_ids_are_unique(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/library/inventory", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_inventory_drive_ids_are_unique(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/library/inventory", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_scan_updates_inventory(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
...
class TestPhysicalMap:
    def test_physical_map_endpoint(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/library/physical", headers=auth_headers)
        assert resp.status_code == 200

    def test_physical_map_has_dimensions(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/library/physical", headers=auth_headers)
        assert resp.status_code == 200
...
class TestMediaList:
    def test_media_list_is_reachable(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/media", headers=auth_headers)
        assert resp.status_code == 200

    def test_media_list_entries_have_barcodes(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/media", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        media = (
            data if isinstance(data, list) else data.get("media") or data.get("cartridges") or []
        )
        if media:
            first = media[0]
