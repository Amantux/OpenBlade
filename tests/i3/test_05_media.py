...
class TestCartridgeLifecycle:
    def test_media_endpoint_returns_list(
    ...
    def test_cartridge_has_required_fields(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/media", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_cartridge_states_are_valid(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/media", headers=auth_headers)
        assert resp.status_code == 200
    ...
    def test_cartridge_detail_by_barcode(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
        resp = i3_client.get("/aml/media", headers=auth_headers)
        assert resp.status_code == 200
        ...
        assert detail_resp.status_code in (200, 404), (
            f"Unexpected status: {detail_resp.status_code}"
        )


class TestMediaPools:
    def test_pools_endpoint_is_reachable(
        self, i3_client: httpx.Client, auth_headers: dict[str, str]
    ) -> None:
    ...
    def test_pools_have_names(self, i3_client: httpx.Client, auth_headers: dict[str, str]) -> None:
        ...
        data = resp.json()
        pools = (
            data if isinstance(data, list) else data.get("partitions") or data.get("pools") or []
        )
        for pool in pools:
